    ...
    <div className="video-player-con">
      {urlVideo ? (
        <div className={`video-container-con ${!isPlaying ? 'paused' : ''}`}>
          <video
            ref={videoRef}
            src={urlVideo?.startsWith('http') ? urlVideo : `http://localhost:3000${urlVideo}`}
          ...
          <div className="video-controls-custom">
            <div className="controls-row">
              <button onClick={skipBackward} className="control-btn" title="Retroceder 10 segundos">
                <SkipBack size={20} />
              </button>
              <button onClick={togglePlay} className="play-btn-custom" title={isPlaying ? "Pausar" : "Reproducir"}>
                {isPlaying ? <Pause size={24} /> : <Play size={24} />}
              </button>
              <button onClick={skipForward} className="control-btn" title="Avanzar 10 segundos">
                <SkipForward size={20} />
              </button>
              <div className="time-display-custom">
                {formatTimeFunction(currentTime)} / {formatTimeFunction(duration)}
              </div>
              <div className="volume-control">
                <button onClick={toggleMute} className="control-btn" title={localIsMuted ? "Activar sonido" : "Silenciar"}>
                  {localIsMuted || volume === 0 ? <VolumeX size={20} /> : <Volume2 size={20} />}
                </button>
                <input
                  type="range"
                  min="0"
              ...
              <button onClick={toggleFullscreen} className="control-btn" title={localIsFullscreen ? "Salir de pantalla completa" : "Pantalla completa"}>
                {localIsFullscreen ? <Minimize size={20} /> : <Maximize size={20} />}
              </button>
            </div>
