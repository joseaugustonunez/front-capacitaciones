...
const InteractionModal = ({
  ...
  const handleSubmit = async () => {
    if (currentInteraction.id_tipo_interaccion === 4 && !textoRespuesta.trim())
      return;
    ...
    try {
      if (currentInteraction.id_tipo_interaccion === 4) {
      ...
      } else if (currentInteraction.id_tipo_interaccion === 2) {
        // Completar espacios
        const espaciosRequeridos =
          currentInteraction.descripcion.split("[]").length - 1;
        ...
        const opcionesSeleccionadas = espaciosCompletados.map((texto) => {
          const opcion = currentInteraction.opciones.find(
            (opt) =>
              opt.texto_opcion.toLowerCase() === texto.toLowerCase().trim()
          );
        ...
        console.log(
          ...
          respuestaUsuario.opciones_seleccionadas
        );
  ...
  const onDragEnd = (result) => {
    ...
    if (
      source.droppableId === "banco-opciones" &&
      destination.droppableId.startsWith("espacio-")
    ) {
      const espacioIndex = parseInt(
        destination.droppableId.replace("espacio-", ""),
        10
      );
    ...
    if (
      source.droppableId.startsWith("espacio-") &&
      destination.droppableId.startsWith("espacio-")
    ) {
      const fromIndex = parseInt(
        source.droppableId.replace("espacio-", ""),
        10
      );
    ...
    if (
      source.droppableId.startsWith("espacio-") &&
      destination.droppableId === "banco-opciones"
    ) {
      const espacioIndex = parseInt(
        source.droppableId.replace("espacio-", ""),
        10
    ...
    return (
      <div
        className={`interaction-message ${
          interactionResult.isCorrect ? "success" : "error"
        }`}
      >
  ...
  const renderContenido = () => {
    ...
    if (currentInteraction.id_tipo_interaccion === 2) {
      return (
                ...
                {index <
                  ...
                  <input
                    ...
                    className={`input-espacio ${
                      espaciosRespuestas[index] ? "has-value" : ""
                    }`}
                      ...
                      >
                        {(provided, snapshot) => (
                          <div
                            ...
                            className={`espacio-drop ${
                              snapshot.isDraggingOver ? "activo" : ""
                            }`}
                          >
                            {respuestaUsuario?.opciones_seleccionadas?.[
                              index
                              ...
                              >
                                {(provided) => {
                                  const opcionSeleccionada =
                                    currentInteraction.opciones.find(
                                      (o) =>
                                        o.id ===
                                        respuestaUsuario.opciones_seleccionadas[
                                          index
                ...
                {(provided) => (
                  ...
                  >
                    {currentInteraction.opciones
                      ...
                      .filter(
                        (op) =>
                          !respuestaUsuario?.opciones_seleccionadas?.includes(
                            op.id
                          )
                      )
                      .map((opcion, index) => (
